...
```mermaid
graph TD
    Docs["Local PDFs / Markdown<br/>(~/ai-knowledge-base, never committed to git)"]
    subgraph Ingestion["INGESTION (ingest.py)"]
        I["Extract → Chunk → Embed → Store"]
    end
    Qdrant["Qdrant<br/>(collection: arivu_kb, 1024-dim, Cosine)"]
    subgraph Query["QUERY (ask.py)"]
        Q["Embed question → Search top-K → Build context → Prompt LLM → Answer (context-grounded)"]
    end

    Docs --> Ingestion --> Qdrant --> Query
```
...
- arivu-rag/
  - config/
    - settings.py single source of truth for all params
  - ingestion/
    - ingest.py parse → chunk → embed → store
  - query/
    - ask.py embed → search → context → generate
